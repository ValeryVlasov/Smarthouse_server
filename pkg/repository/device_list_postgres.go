...
type DeviceListPostgres struct {
	db *sqlx.DB
}

func NewDeviceListPostgres(db *sqlx.DB) *DeviceListPostgres {
	return &DeviceListPostgres{db: db}
}

func (r *DeviceListPostgres) Create(userId int, list Smarthouse_server.DeviceList) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	var id int
	createListQuery := fmt.Sprintf("INSERT INTO %s (title, description) VALUES ($1, $2) RETURNING id", deviceListsTable)
...
func (r *DeviceListPostgres) Update(userId, listId int, input Smarthouse_server.UpdateListInput) error {
